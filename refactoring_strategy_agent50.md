# Refactoring Strategy: CPU Kernels (NN & Control Flow) (Agent 50)

## Current Status
An initial analysis of the repository shows that there are currently **no C++ source files** (`*.cc`, `*.cpp`, `*.h`, `*.hpp`) present in this submodule for the CPU Kernels (NN & Control Flow). The workspace only contains the refactoring plan documents, scripts, and some base C files for strings and vectors (`tf_string.h`, `tf_string.c`, `tf_vector.h`, `tf_vector.c` and their tests).

## Action Plan (Once files are available)
When the C++ files for the CPU Kernels (NN & Control Flow) are added, the following strict refactoring rules will be applied to rewrite the codebase to strict C99/C11:
...
2. **Function Signatures**:
   ...
   - The first argument of these functions will be a pointer to the respective `struct` (e.g., `Kernel_execute(Kernel* k)`).
