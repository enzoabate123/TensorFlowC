# Refactoring Strategy: GPU/TPU Device Integration (Part 1 of 5)
...
When the C++ files for the GPU/TPU Device Integration (Part 1) are added, the following strict refactoring rules will be applied to rewrite the codebase to strict C99/C11:
...
3. **Data Structures**:
   ...
   - We will use manually managed C buffers, `char*`, and dynamically allocated arrays (`malloc`/`calloc`/`realloc`). Alternatively, use the existing `TF_String` and `TF_Vector` implementations provided.
