# Refactoring Strategy: Graph Representation & Parsing (Part 2 of 10)
...
When the C++ files for the Graph Representation & Parsing (Part 2) are added, the following strict refactoring rules will be applied to rewrite the codebase to strict C99/C11:
...
3. **Data Structures**:
   - Standard C++ library structures like `std::string` and `std::vector` will be replaced using manually managed C buffers and dynamic arrays, similar to `tf_string.c` and `tf_vector.c`.
