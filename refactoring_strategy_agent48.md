# Refactoring Strategy: CPU Kernels (NN & Control Flow) (Parte 8 de 10)

## Current Status
An initial analysis of the repository shows that there are currently **no C++ source files** (`*.cc`, `*.cpp`, `*.h`, `*.hpp`) present in this submodule.

## Action Plan (Once files are available)
When the C++ files for the CPU Kernels (NN & Control Flow) (Part 8) are added, the following strict refactoring rules will be applied to rewrite the codebase to strict C99/C11:
