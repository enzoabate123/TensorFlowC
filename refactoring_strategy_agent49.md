# Refactoring Strategy: CPU Kernels (NN & Control Flow) (Part 9 of 10) - Agent 49
...
When the C++ files for the CPU Kernels (NN & Control Flow) (Part 9) are added, the following strict refactoring rules will be applied to rewrite the codebase to strict C99/C11:
...
3. **Data Structures**:
   ...
   - We will use manually managed C buffers, `char*`, and dynamically allocated arrays (`malloc`/`calloc`/`realloc`). We will leverage the `TF_String` and `TF_Vector` implementations.
