...
1. **Remove Object-Oriented Paradigms**:
   ...
   - C++ classes representing components will be converted to C `struct`s.
...
3. **Data Structures**:
   ...
   - We will use manually managed C buffers, `char*`, and dynamically allocated arrays (`malloc`/`calloc`/`realloc`). We will leverage the newly created string and vector implementations found in `tf_string.c` and `tf_vector.c` if applicable.
