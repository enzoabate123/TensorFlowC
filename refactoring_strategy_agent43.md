# Refactoring Strategy: CPU Kernels (NN & Control Flow) (Part 3 of 10) - Agent 43
...
When the C++ files for the CPU Kernels (NN & Control Flow) (Part 3) are added, the following strict refactoring rules will be applied to rewrite the codebase to strict C99/C11:
...
3. **Data Structures**:
   ...
   - We will use manually managed C buffers, `char*`, and dynamically allocated arrays (`malloc`/`calloc`/`realloc`). We can utilize the existing `TF_String` and `TF_Vector` implementations if suitable.
...
5. **Testing**:
   ...
   - Tests will be compiled manually using gcc with strict flags and execution command will include memory leak checks using Valgrind.
